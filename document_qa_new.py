...
def build_rag_chain_from_text(text, token_name, filename, level='None'):
    ...
    for i in documents_vectores:
        ...
        if level != 'None':
            metadata_list.append(
                {"source": i['searchable_text_type'], "searchable_text": i['searchable_text'], "filename": filename,
                 'level': level})
        else:
            metadata_list.append(
                {"source": i['searchable_text_type'], "searchable_text": i['searchable_text'], "filename": filename})
    ...
    if len(document_list) > 40000:
        ...
        for i in range(block):
            ...
            collection.add(documents=document_list[start:end], embeddings=embedding_list[start:end],
                           metadatas=metadata_list[start:end], ids=id_list[start:end])
        collection.add(documents=document_list[block * 40000:-1], embeddings=embedding_list[block * 40000:-1],
                       metadatas=metadata_list[block * 40000:-1], ids=id_list[block * 40000:-1])
    else:
...
def document_search(question, token_name, fragement_num, level='None'):
    ...
    if level != 'None':
        fragement_candidates = \
            collection.query(query_embeddings=[query_embedding], n_results=1, where={"level": level})['documents']
        tmp_searchable_text = \
            collection.query(query_embeddings=[query_embedding], n_results=1, where={"level": level})['metadatas'][0]
    else:
        ...
        if conf['application']['name'] == 'the_line':
            tmp_all_tasks = \
                collection.query(query_embeddings=[query_embedding], n_results=1, where={"source": "QA_pairs"})[
                    'documents']
        else:
            tmp_all_tasks = \
                collection.query(query_embeddings=[query_embedding], n_results=1, where={"source": "All_texts"})[
                    'documents']
...
def answer_from_doc(token_name, question, chat_id, msg_id, stream=True, level='None'):
    ...
    # llm_dict["llm"] = i
    if level != 'None':
        fragement_candidates, searchable_text, basic_qa, other_candidate = document_search(question, token_name,
                                                                                           fragement_num, level)
    else:
        fragement_candidates, searchable_text, basic_qa, other_candidate = document_search(question, token_name,
                                                                                           fragement_num)
    ...
    if basic_qa == 3:
        ...
        return response, fragement_candidates, similarity_score, filename, "qa_pair", False
    ...
    if similarity_score < 0.4 and other_candidate != '':
        prompt = prompter.generate_prompt(question=question, context=other_candidate,
                                          prompt_serie=conf['prompt']['prompt_serie'])
        fragement_candidates = other_candidate
        basic_qa = 0
        logger.info(f"USE ALL TEXTS!")
    else:
        prompt = prompter.generate_prompt(question=question, context=context_fragements,
                                          prompt_serie=conf['prompt']['prompt_serie'])

    _origin = "generative_llm"
    _stream = True
...
Let's work together to foster a culture of safety excellence. If you have any questions or need assistance, feel free to ask, and let's build a safer tomorrow!"""
    ...
    try:
        fragement_candidates = fragement_candidates[0][0] if type(
            fragement_candidates) is not str else fragement_candidates
    except:
        pass
    if basic_qa == 1:
        return response, '', None, '', _origin, _stream
    else:
        return response, fragement_candidates, similarity_score, filename, _origin, _stream
